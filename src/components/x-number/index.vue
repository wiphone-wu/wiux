<template>
  <div class="weui-cell">
    <div>
      <p v-html="title" :style="labelStyle" :class="labelClass"></p>
    </div>
    <div
      v-show="!readonly"
      class="weui-cell__ft wiux-cell-primary"
      :class="{ 'wiux-number-round': buttonStyle === 'round' }"
      style="font-size: 0"
    >
      <div :style="({ float: align } as any)">
        <a
          @click="sub"
    ...
    <div v-show="readonly" class="weui-cell__ft wiux-cell-primary">
      {{ modelValue }}
    </div>
  </div>
</template>
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const labelStyle = computed(() => {
      return {
        width: parentLabelWidth.value || parent.value.labelWidth,
        textAlign: parentLabelAlign.value || parent.value.labelAlign,
        marginRight:
          parentLabelMarginRight.value || parent.value.labelMarginRight,
      };
    });

    const labelClass = computed(() => {
      const align =
        parentLabelAlign.value || (parent.value && parent.value.labelAlign);
      return {
        "wiux-cell-justify":
          align === "justify" ||
          (parent.value.$parent &&
            parent.value.$parent.labelAlign === "justify"),
      };
    });
